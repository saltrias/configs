...
function M.lualineAutoPalette(colorPallete, colorOffset, colorStep)
    ...
    local n = #colorPallete
    local textColor = "#181825"
    local textColorOnDark = "#CDD6F4"

    for i, mode in ipairs(modes) do
        local a = { bg = colorPallete[((colorOffset + (i - 1) * colorStep) % n) + 1], fg = textColor, gui="bold" }
        local b = { bg = colorPallete[((colorOffset + (i - 0) * colorStep) % n) + 1], fg = textColor }
        local c = { bg = textColor, fg = textColorOnDark }
        local x = { bg = textColor, fg = textColorOnDark }
        local y = { bg = colorPallete[((colorOffset + (i + 1) * colorStep) % n) + 1], fg = textColor }
        local z = { bg = colorPallete[((colorOffset + (i + 2) * colorStep) % n) + 1], fg = textColor }
